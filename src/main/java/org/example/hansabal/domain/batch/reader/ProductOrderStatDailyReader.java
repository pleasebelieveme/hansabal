...
@Component
public class ProductOrderStatDailyReader implements ItemReader<Order> {
	...
	public Order read() {
		...
		if (orderIterator == null) {
			...
			// 완료된 주문 상태(OrderStatus.FINISHED)이며, 생성일자가 from~to 사이인 주문들을 조회
			List<Order> orders = orderRepository.findAllByStatusAndCreatedDateRange(OrderStatus.FINISHED, from, to);
