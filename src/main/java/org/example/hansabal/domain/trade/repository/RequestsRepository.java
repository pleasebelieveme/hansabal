...
public interface RequestsRepository extends JpaRepository<Requests, Long> , RequestsRepositoryCustom{
	...
	// @Query(value="SELECT r FROM Requests r WHERE r.deletedAt IS null AND r.trade.id=:tradeId  Trade BY r.id asc",
	// 	countQuery= "SELECT COUNT(r) FROM Requests r WHERE r.trade.id=:tradeId AND r.deletedAt IS null")
	// Page<RequestsResponseDto> findByTradeIdTradeByRequestsIdAsc(@Param("tradeId")Long tradeId,Pageable pageable);
}
